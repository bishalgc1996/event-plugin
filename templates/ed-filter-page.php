								...
								if ( $loop_one->have_posts() ) {
									...
									while ( $loop_one->have_posts() ):
										...
										$start_date_str
											                 = get_post_meta( get_the_ID(),
											'start_ts', true );
										$start_date_formated = date_i18n( 'F j',
											$start_date_str );
										if ( $start_date_formated
										     && ! in_array( $start_date_formated,
												$date_array )
										) {
											$date_array[]
												= $start_date_formated;
        ...
        <div class="events-listing">
            <div class="row d-flex" data-js-filter="target">
				...
				while ( $loop->have_posts() ):
					$loop->the_post();
					$venue['state']      = get_post_meta( get_the_ID(),
						'venue_state', true );
					$start_date_str      = get_post_meta( get_the_ID(),
						'start_ts', true );
					$start_date_formated = date_i18n( 'F j Y',
						$start_date_str );
					$event_url           = get_post_meta( get_the_ID(),
						'ed_event_link', true );
					$ed_options          = get_option( ED_OPTIONS );
					$time_format         = isset( $ed_options['time_format'] )
						? $ed_options['time_format'] : '12hours';
